...
import { socketService, SOCKET_EVENT_WATCHBOARD ,SOCKET_EMIT_UPDATEBOARD} from '../services/socketService'
...
export const boardStore = {
  ...
  mutations: {
    ...
    updateGroup(state,{group}){
      let idx = state.boards.groups.findIndex((currGroup)=>currGroup._id===group._id)
      state.currBoard.groups.splice(idx,1,group)
    },
    removeGroup(state,{groupId}){
     let idx = state.currBoard.groups.findIndex((currGroup)=>currGroup._id===groupId)
  ...
  async addGroup({dispatch,commit},{group}){
    ...
    async getTaskById(){    //Not sure if we need it yet
    try{
