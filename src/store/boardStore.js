...
export const boardStore = {
  ...
  mutations: {
    ...
    updateStore(state, { boardId, taskId }) {
      ...
      let groupIdx = state.boards[boardIdx].groups.findIndex((group)=> group.tasks.some((task)=> task.id === taskId))
    ...
    updateTask(state, { task }) {
      const groupIdx = state.currBoard.groups.findIndex(
        (currGroup) => currGroup.tasks.find((currTask)=>task.id === currTask.id)
      );
